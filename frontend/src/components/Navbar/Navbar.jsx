...
const Navbar = ({ setShowLogin }) => {
  ...
  return (
    <div className={`navbar${mobileMenuOpen ? ' mobile-open' : ''}`}> 
      <Link to='/'><img className='logo transition-transform duration-300 hover:scale-105' src={assets.logo} alt="Logo" /></Link>
      <button className="navbar-hamburger transition-transform duration-200 hover:scale-110" aria-label="Open menu" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
        <span className="bar"></span>
        <span className="bar"></span>
        <span className="bar"></span>
      </button>
      <ul className={`navbar-menu${mobileMenuOpen ? ' show' : ''} text-base md:text-lg`}> 
      ...
      <div className="navbar-right">
        <Link to='/cart' className='navbar-search-icon transition-transform duration-200 hover:scale-110' tabIndex={0} aria-label="Cart">
          <img src={assets.basket_icon} alt="Cart" />
          <div className={getTotalCartAmount() > 0 ? "dot" : ""}></div>
        </Link>
        { !token ? <button onClick={() => setShowLogin(true)} tabIndex={0} className="transition-transform duration-200 hover:scale-105">Sign in</button>
