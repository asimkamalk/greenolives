...
const StoreContextProvider = (props) => {
  ...
  const fetchUserData = async () => {
    if (token) {
      try {
        console.log(
          "Making API call to fetch user data with token:",
  ...
  const fetchFavorites = async () => {
    if (!token) return;
    try {
      const response = await axios.get(url + "/api/user/favorites", {
        headers: { token },
      });
      if (response.data.success) {
